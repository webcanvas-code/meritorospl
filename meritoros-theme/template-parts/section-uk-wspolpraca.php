...
$title = mer_field('uk_wsp_title', 'Jak wygląda bieżąca współpraca');
...
$s2_title = is_array($s2) && !empty($s2['title']) ? $s2['title'] : 'Elastyczne zamknięcie miesiąca';
$s2_lead  = is_array($s2) && !empty($s2['lead'])  ? $s2['lead']  : 'Terminy zamknięcia miesiąca ustalamy indywidualnie z każdą firmą, uwzględniając jej wewnętrzne potrzeby raportowe oraz obowiązujące terminy podatkowe.';
...
$s3_title = is_array($s3) && !empty($s3['title']) ? $s3['title'] : "Zakres i częstotliwość raportowania\nustalamy indywidualnie z każdym klientem.";
...
$s3_note  = is_array($s3) && !empty($s3['note'])  ? $s3['note']  : 'W zależności od potrzeb przygotowujemy również dodatkowe raporty księgowe, finansowe lub kadrowo-płacowe.';

$btn_text = mer_field('uk_wsp_btn_text', 'Poznaj więcej historii');
                    ...
                    <div class="flex-1">
                        ...
                        <ul class="space-y-3 text-slate-600">
                            ...
                            <li class="flex items-start gap-3">
                                <i data-lucide="check-circle-2" class="w-5 h-5 text-[#2d8650] shrink-0 mt-0.5"></i>
