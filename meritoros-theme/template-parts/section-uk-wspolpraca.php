...
$title = __( mer_field('uk_wsp_title', 'Jak wygląda bieżąca współpraca'), 'meritoros' );

$s1 = get_field('uk_wsp_step1') ?: ($_orig_id !== $_page_id ? get_field('uk_wsp_step1', $_orig_id) : null);
...
$s1_items_raw = is_array($s1) && !empty($s1['items']) ? $s1['items'] : "na bieżąco – obsługując codzienne procesy księgowe lub kadrowe\nw cyklach tygodniowych\nw innych ustalonych odstępach czasu";

$s2 = get_field('uk_wsp_step2') ?: ($_orig_id !== $_page_id ? get_field('uk_wsp_step2', $_orig_id) : null);
$s2_title = __( is_array($s2) && !empty($s2['title']) ? $s2['title'] : 'Elastyczne zamknięcie miesiąca', 'meritoros' );
$s2_lead  = __( is_array($s2) && !empty($s2['lead'])  ? $s2['lead']  : 'Terminy zamknięcia miesiąca ustalamy indywidualnie z każdą firmą, uwzględniając jej wewnętrzne potrzeby raportowe oraz obowiązujące terminy podatkowe.', 'meritoros' );
$s2_items_raw = is_array($s2) && !empty($s2['items']) ? $s2['items'] : "część firm potrzebuje raportów finansowych do 20. dnia miesiąca\ninne wymagają wyników już w 3. lub 4. dniu roboczym nowego miesiąca";

$s3 = get_field('uk_wsp_step3') ?: ($_orig_id !== $_page_id ? get_field('uk_wsp_step3', $_orig_id) : null);
$s3_title = __( is_array($s3) && !empty($s3['title']) ? $s3['title'] : "Zakres i częstotliwość raportowania\nustalamy indywidualnie z każdym klientem.", 'meritoros' );
$s3_lead  = __( is_array($s3) && !empty($s3['lead'])  ? $s3['lead']  : 'W standardzie klient otrzymuje:', 'meritoros' );
$s3_items_raw = is_array($s3) && !empty($s3['items']) ? $s3['items'] : "rachunek zysków i strat\nbilans\nzestawienie należności i zobowiązań";
$s3_note  = __( is_array($s3) && !empty($s3['note'])  ? $s3['note']  : 'W zależności od potrzeb przygotowujemy również dodatkowe raporty księgowe, finansowe lub kadrowo-płacowe.', 'meritoros' );

$btn_text = __( mer_field('uk_wsp_btn_text', 'Poznaj więcej historii'), 'meritoros' );
$btn_url  = mer_field('uk_wsp_btn_url',  home_url('/blog/'));
...
<section id="wspolpraca" class="py-16 md:py-24 bg-emerald-50 relative">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
                    ...
                    <div class="flex-1">
                        ...
                        <ul class="space-y-3 text-slate-600">
                            ...
                            <li class="flex items-start gap-3">
                                <i data-lucide="check-circle-2" class="w-5 h-5 text-[#00d084] shrink-0 mt-0.5"></i>
        ...
        <div class="mt-20 text-center">
            <a href="<?php echo esc_url($btn_url); ?>" class="mer-btn mer-btn--white px-7 py-3.5 rounded-full bg-white border border-slate-200 text-slate-700 text-base font-semibold hover:border-slate-300 hover:bg-slate-50 transition-colors inline-block">
