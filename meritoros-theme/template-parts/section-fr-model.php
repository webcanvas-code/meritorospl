...
<style>
    .fr-flip-card { perspective: 1000px; cursor: pointer; }
    .fr-flip-card-inner { position: relative; width: 100%; height: 100%; transform-style: preserve-3d; transition: transform 0.65s cubic-bezier(.4,0,.2,1); }
    @media (hover: hover) {
        .fr-flip-card:hover .fr-flip-card-inner { transform: rotateY(180deg); }
    }
    .fr-flip-card.flipped .fr-flip-card-inner { transform: rotateY(180deg); }
    .fr-flip-card-front, .fr-flip-card-back { position: absolute; inset: 0; backface-visibility: hidden; -webkit-backface-visibility: hidden; border-radius: 1.5rem; overflow: hidden; }
    .fr-flip-card-back { transform: rotateY(180deg); }
    .fr-flip-hint { display: none; }
    @media (hover: none) { .fr-flip-hint { display: flex; } }
</style>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="fr-flip-card h-[380px]">
                <div class="fr-flip-card-inner">
                    ...
                    <div class="fr-flip-card-back bg-white border border-slate-200 flex flex-col justify-center p-12 relative">
            ...
            <div class="fr-flip-card h-[380px]">
                <div class="fr-flip-card-inner">
                    <div class="fr-flip-card-front relative flex items-center justify-center text-center">
                        ...
                        <h3 class="text-3xl font-bold tracking-tight text-white relative z-10 px-8"><?php echo nl2br(esc_html($m2_title)); ?></h3>
                        <span class="fr-flip-hint relative z-10 items-center gap-1.5 mt-4 text-xs text-white/60">
                            <i data-lucide="hand-metal" class="w-3.5 h-3.5"></i> Dotknij, aby zobaczyć więcej
                        </span>
                    </div>
                    <div class="fr-flip-card-back relative flex flex-col justify-center p-12">
                        <img src="<?php echo $m2_img_url; ?>" alt="<?php echo $m2_img_alt; ?>" class="absolute inset-0 w-full h-full object-cover" loading="lazy">
...
<script>
document.querySelectorAll('.fr-flip-card').forEach(function (card) {
