<?php
function uk_video_embed_url(string $url): string {
    if (!$url) return '';
...
function uk_video_thumbnail(string $url): string {
    if (preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/', $url, $m)) {
...
$slide_defaults = [
    1 => [
        ...
        'desc'       => 'Po kilku zmianach głównej księgowej spółka potrzebowała szybkiego uporządkowania księgowości i bezpiecznego zamknięcia roku obrotowego.',
    ...
    2 => [
        ...
        'desc'       => 'Dynamicznie rosnąca spółka technologiczna potrzebowała partnera, który zapewni rzetelną sprawozdawczość i gotowość do pozyskania inwestora.',
...
for ($i = 1; $i <= 2; $i++) {
    ...
    $img_url = is_array($image) ? esc_url($image['url']) : (uk_video_thumbnail($video_url) ?: $d['img']);
    ...
    if (is_array($video_file) && !empty($video_file['url'])) {
        ...
        $play_src  = esc_attr(uk_video_embed_url($video_url));
        $play_type = 'embed';
    } else {
        $play_src  = '';
        $play_type = '';
...
<section class="py-10 md:py-20 bg-white relative">
    <div class="max-w-7xl mx-auto px-6 mb-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2 shrink-0">
            <button id="uk-historie-prev" type="button" class="w-12 h-12 rounded-full bg-white border border-slate-200 flex items-center justify-center shadow-sm hover:bg-slate-50 transition-colors" style="opacity:0.35;pointer-events:none">
                <i data-lucide="chevron-left" class="w-5 h-5 text-slate-700 stroke-[2]"></i>
            </button>
            <button id="uk-historie-next" type="button" class="w-12 h-12 rounded-full bg-[#00d084] flex items-center justify-center shadow-md hover:bg-[#00b872] transition-colors">
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div id="uk-historie-track" class="flex gap-6 transition-transform duration-500 ease-in-out">
            <?php foreach ($slides as $idx => $slide) : ?>
                <div class="min-w-[85%] sm:min-w-[calc(50%-12px)] lg:w-[400px] lg:min-w-[400px] rounded-2xl overflow-hidden border border-slate-200 flex flex-col">
                    <div class="relative h-[155px] shrink-0">
                        ...
                        <button class="absolute inset-0 flex items-center justify-center group uk-play-btn"
                    ...
                    <div class="p-5 flex flex-col flex-1">
                        ...
                        <a href="<?php echo esc_url($btn_url); ?>" class="mt-auto inline-flex px-5 py-2.5 rounded-full bg-white text-slate-700 text-xs font-semibold border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-colors self-start">
    ...
    <div class="max-w-7xl mx-auto px-6 mt-6 flex items-center justify-center gap-2" id="uk-historie-dots">
        <?php foreach ($slides as $idx => $slide) : ?>
...
<!-- Video modal -->
<div id="uk-video-modal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4 md:p-10">
    <div class="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" id="uk-modal-backdrop"></div>
    <div class="relative w-full max-w-4xl z-10">
        <button id="uk-modal-close" class="absolute -top-10 right-0 text-white/80 hover:text-white transition-colors flex items-center gap-1.5 text-sm">
            <i data-lucide="x" class="w-5 h-5"></i> <?php echo esc_html__('Zamknij', 'meritoros'); ?>
        </button>
        <div id="uk-embed-wrap" class="relative w-full hidden" style="padding-bottom:56.25%">
            <iframe id="uk-video-iframe" class="absolute inset-0 w-full h-full rounded-2xl" src="" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>
        </div>
        <div id="uk-file-wrap" class="hidden">
            <video id="uk-video-file" controls autoplay playsinline class="w-full rounded-2xl bg-black max-h-[80vh]" src=""></video>
        </div>
    </div>
</div>
...
<script>
(function () {
    var track   = document.getElementById('uk-historie-track');
    var nextBtn = document.getElementById('uk-historie-next');
    var prevBtn = document.getElementById('uk-historie-prev');
    var dots    = document.querySelectorAll('#uk-historie-dots button');
    if (!track) return;
    ...
    var modal    = document.getElementById('uk-video-modal');
    var backdrop = document.getElementById('uk-modal-backdrop');
    var closeBtn = document.getElementById('uk-modal-close');
    var embedWrap = document.getElementById('uk-embed-wrap');
    var fileWrap  = document.getElementById('uk-file-wrap');
    var iframe    = document.getElementById('uk-video-iframe');
    var videoEl   = document.getElementById('uk-video-file');
    ...
    document.querySelectorAll('.uk-play-btn').forEach(function (btn) {
